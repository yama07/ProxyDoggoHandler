...
import { Button, Toolbar, TableHead, Table, TableRow, TableCell, TableBody } from '@material-ui/core';
...
const ProxiesPreferencesContainer: React.FC = () => {
    ...
    const addSetting = React.useCallback((icon: string, name: string,
    ...
    const deleteSetting = React.useCallback((index: number) => {
        setUpstreams([...upstreams.slice(0, index), ...upstreams.slice(index + 1)]);
    ...
    return (
        <main className={classes.content}>
            ...
            <Table size="small">
                ...
                <TableBody>
                    {upstreams.map((upstream, index) => (
                        <TableRow key={index}>
                            <TableCell align="center"><DogBreadsIcon iconId={upstream.icon} /></TableCell>
                            <TableCell>{upstream.name}</TableCell>
                            <TableCell>{upstream.connectionSetting?.host ?? ""}</TableCell>
                            <TableCell align="center">{upstream.connectionSetting?.credentials != null ? "Yes" : ""}</TableCell>
                            {/* {0 < index && <TableCell><EditIcon onClick={() => { handleEdit(index) }} /></TableCell>} */}
                            {0 < index && <TableCell><DeleteIcon onClick={() => { handleDelete(index) }} /></TableCell>}
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
            ...
            <DeleteProxyDialog
                ...
                onConfirm={() => { deleteSetting(selectedIndex) }}
            />
        </main>
    );
};
