...
import { getGeneralPreference, getProxiesPreference, setProxiesPreference } from "./preference-accessor";
import { init, listen, updateUpstreamProxyUrl } from "./proxy-chain-wrapper";

let tray: Tray = null;

const getIconPath = (iconId: string): string => (
    path.join(__dirname, "tray-icons", "dog-breeds", iconId + (process.platform === 'win32' ? '.ico' : '.png'))
);

export const updateTray = () => {
    const proxiesPreference = getProxiesPreference();

    const proxyMenuItems = proxiesPreference.upstreams.map((proxy, index) => (
        new MenuItem({
            label: proxy.name,
            type: 'radio',
            ...
            click: (item, window, event) => {
                ...
                updateTray();
                updateUpstreamProxyUrl(proxiesPreference.upstreams[index].connectionSetting);
            },
        })
    ));
    ...
    const iconPath = getIconPath(proxiesPreference.upstreams[proxiesPreference.selectedIndex].icon);
